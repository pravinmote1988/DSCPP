...
{
    public:
        bool cycleInDfs(int node, int parent, vector<vector<int>> &adjList, vector<int> &visited) 
        {
            if(!visited[node])
            {
                visited[node] = 1;
                for(int vertex : adjList[node]) 
                {
                    if(!visited[vertex]) 
                    {
                        if(cycleInDfs(vertex, node, adjList, visited))
                            return true;
                    }
...
int main()
{
    ...
    {
        if(!visited[i])
           result = s.cycleInDfs(i, -1, adjList, visited);
        if(result)
                break;
    }
